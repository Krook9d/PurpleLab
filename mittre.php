...
<?php
    ...
    $filePath = '/var/www/html/Mittre/Technic/list.txt';
    $results = [];
...
<?php
if (isset($_GET['run-test'])) {
    $command = $_GET['run-test'];
    $output = shell_exec("bash /var/www/html/run_commands.sh $command");
    ...
    <script>
        ...
        function fetchTest(testName) {
            if (lastTestNameClicked === testName) {
                ...
                var url = 'http://192.168.142.130/Mittre/Technic/' + testName + '/' + testName + '.md';
...
// Call the PHP script to execute the command
    $.get('mittre.php', { command: 'command_1' }, function(data) {
        console.log(data);
    });
